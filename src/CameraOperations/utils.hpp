...
inline cv::Scalar getColorForColorId_u(int colorId) {
    switch (colorId)
    {
    case 1:
        return cv::Scalar(250, 5, 5, 255); //blue
        break;
    case 2:
        return cv::Scalar(5, 190, 30, 255); //green
        break;
    case 3:
        return cv::Scalar(10, 170, 250, 255); //orange
        break;
    case 4:
        return cv::Scalar(25, 25, 240, 255); //red
        break;
    default:
        break;
    }

    return cv::Scalar(170, 170, 170, 255); //gray
}
inline sl::float4 getColorForColorId_f(int colorId) {
    auto clr_u = getColorForColorId_u(colorId);
    return sl::float4(static_cast<float>(clr_u.val[0]) / 255.f, static_cast<float>(clr_u.val[1]) / 255.f, static_cast<float>(clr_u.val[2]) / 255.f, 1.f);
}
