...
void render_2D(cv::Mat &left_display, sl::float2 img_scale, std::vector<sl::ObjectData> &objects, std::map<int, int> colors, bool showIgnored, bool render_mask) {
    ...
    for (auto& obj : objects) {
        int colorId = colors.find(obj.id)->second; 
        //color id == 1 is ignore
        if(renderObject(obj) && (colorId != 1 || showIgnored)) {
            cv::Scalar base_color = getColorForColorId_u(colorId);
            ...
            putText(left_display,  toString(obj.label).get(), cv::Point2d(position_image.x - 20, position_image.y - 15),
                        cv::FONT_HERSHEY_COMPLEX_SMALL, 2.0, cv::Scalar(255, 255, 255, 255), 2,5 );

            if (std::isfinite(obj.position.z)) {
                char text[64];
                sprintf(text, "%2.1fM", abs(obj.position.z / 1000.0f));
                putText(left_display, text, cv::Point2d(position_image.x - 20, position_image.y + 15),
                        cv::FONT_HERSHEY_COMPLEX_SMALL, 2.0, cv::Scalar(255, 255, 255, 255), 2,5 );
            }
        }
    }
...
void TrackingViewer::generate_view(sl::Objects &objects, std::map<int, int> colorMap, sl::Pose current_camera_pose, cv::Mat &tracking_view, bool tracking_enabled) {
    ...
    if (tracking_enabled) {
        ...
        // Draw all tracklets
        drawTracklets(tracking_view, current_camera_pose, colorMap);
    } else {
...
void TrackingViewer::drawTracklets(cv::Mat &tracking_view, sl::Pose current_camera_pose, std::map<int, int> colorMap) {
    for (const Tracklet track : tracklets) {
        ...
        auto clr = getColorForColorId_u(colorMap.find((int)track.id)->second);
