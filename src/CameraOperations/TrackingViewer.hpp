...
void render_2D(cv::Mat &left, sl::float2 img_scale, std::vector<sl::ObjectData> &objects, std::map<int, int> colors, bool showIgnored, bool render_mask = false);
...
public:
    ...
    void generate_view(sl::Objects &objects, std::map<int, int> colorMap, sl::Pose current_camera_pose, cv::Mat &tracking_view, bool tracking_enabled);
...
private:
    ...
    void drawTracklets(cv::Mat &tracking_view, sl::Pose current_camera_pose, std::map<int, int> colorMap);
